...
<link id="theme-stylesheet" rel="stylesheet" href="./css/chofer/tarifa.css">
<script src="./js/chofer/tarifa.js"></script>
...
<aside class="mdc-drawer" style="height: auto">
    ...
    <div class="mdc-drawer__content">
        <nav class="mdc-list">
            ...
            <a id="item-selected" class="mdc-list-item" href="./tarifa-chofer">
                <span class="mdc-list-item__ripple"></span>
                <i id="item-icon-selected" class="material-icons mdc-list-item__graphic" aria-hidden="true">paid</i>
                <span id="item-text-selected" class="mdc-list-item__text">Tarifas</span>
            </a>
...
<div class="container-fluid mh-100 min-vh-100" >
    <div class="row align-items-center min-vh-100">
        <div id="login-form" class="col" style="justify-content: center; max-height: 100vh; overflow: auto">
            <h1 style="font-size: 4rem; font-weight: 700; margin: 24px;">Tarifas</h1>
            <div class="mdc-card" style="margin: 24px">
                <div class="card-title" style="font-size: 2.3rem; line-height: 2rem; font-weight: 700; margin: 16px">
                    Traslado
                </div>
                @foreach($data['tarifaTraslado'] as $tarifa)
                    <div class="color-text" style="font-size: 1.25rem; line-height: 2rem; font-weight: 500; margin: 0px 16px 32px 16px">
                        Cada {{ $tarifa->kilometro }} kilómetro(s) tiene un costo de {{ $tarifa->precio }}Bs.
                    </div>
                @endforeach
            </div>
            <div class="mdc-card" style="margin: 24px">
                ...
                @foreach($data['tarifaMudanza'] as $tarifa)
                    <div class="color-text" style="font-size: 1.25rem; line-height: 2rem; font-weight: 500; margin: 0px 16px 32px 16px">
                        Cada {{ $tarifa->kilometro }} kilómetro(s) tiene un costo de {{ $tarifa->precio }}Bs.
                    </div>
                @endforeach
            ...
            <div class="mdc-card" style="margin: 24px">
                ...
                @foreach($data['tarifaViaje'] as $tarifa)
                    <div class="color-text" style="font-size: 1.25rem; line-height: 2rem; font-weight: 500; margin: 0px 16px 32px 16px">
                        Cada {{ $tarifa->kilometro }} kilómetro(s) tiene un costo de {{ $tarifa->precio }}Bs.
                    </div>
                @endforeach
        ...
        <div id="second-content" class="col-md" style="display: flex; justify-content: center; flex-direction: column; height: 100vh">
            <img src="./assets/tarifa.png" alt="Welcome" class="img-fluid" style="padding: 64px">
        </div>
    </div>
</div>
</body>
